...
# Dataset de treino (para limites reais do mapa)
TRAIN_DATA_FILE = "dados_treino.csv"

# Colunas reais no teu CSV (confirmadas por ti)
COL_DUREZA_TREINO = "mg CaCO3/L"
COL_PH_TREINO = "pH_dia_evento"

# Limiares do "semáforo" (visual)
...
def carregar_modelo(path: str):
    ...
    features = bundle.get("features", ["Dureza", "pH_evento"])
...
def carregar_limites_reais(train_file: str):
    ...
    d_min, d_max = float(df2[COL_DUREZA_TREINO].min()), float(df2[COL_DUREZA_TREINO].max())
    p_min, p_max = float(df2[COL_PH_TREINO].min()), float(df2[COL_PH_TREINO].max())
...
def plot_mapa_risco_com_contornos(
    ...
    # Contornos (fronteiras do semáforo)
    ...
    # Retângulo do domínio do treino
...
st.write(
    "Ferramenta para estimar o risco de formação de depósitos (proxy: recuperação prolongada do pH) "
    "a partir de medições de **dureza da água** e **pH no dia do evento**."
)

if not os.path.exists(MODEL_FILE):
    st.error(f"Não foi encontrado o ficheiro do modelo: '{MODEL_FILE}'. Coloca-o na mesma pasta desta app.")
    st.stop()

model, threshold, _ = carregar_modelo(MODEL_FILE)

limites = carregar_limites_reais(TRAIN_DATA_FILE)

with st.sidebar:
    st.header("⚙️ Definições")
    st.caption("Ajustes de visualização (não alteram o modelo).")
    ...
    n_grid = st.slider("Resolução do mapa (mais alto = mais lento)", 25, 160, 90, 5)

    st.caption(f"Limiar binário interno do modelo (treino): **{threshold:.2f}**")
...
# Aviso de extrapolação (fora do treino)
fora = False
if limites is not None:
    ...
    if fora:
        ...
        st.warning(
            "⚠️ Atenção: " + " e ".join(msgs) +
            ". A previsão corresponde a extrapolação e pode ser menos fiável."
        )
    else:
        st.caption(
            f"Valores dentro do domínio do treino: dureza [{limites['dureza_min']:.1f}, {limites['dureza_max']:.1f}] | "
            f"pH [{limites['ph_min']:.2f}, {limites['ph_max']:.2f}]"
        )
else:
    st.info("Nota: não foi possível carregar os limites do treino a partir de 'dados_treino.csv'.")
...
if btn:
    ...
    if risco == "Baixo":
        ...
        st.error("🔴 Risco **Alto** de formação de depósitos.")
...
st.subheader("3) Mapa de risco (com domínio do treino)")

if limites is None:
    st.warning("Não foi possível obter os limites reais a partir de 'dados_treino.csv'. A usar limites aproximados.")
    ...
    # margem para o mapa não ficar colado
    ...
    st.caption(
        f"Domínio do treino: dureza [{limites['dureza_min']:.1f}, {limites['dureza_max']:.1f}] | "
        f"pH [{limites['ph_min']:.2f}, {limites['ph_max']:.2f}]"
    )
...
st.download_button(
    "⬇️ Descarregar histórico (CSV)",
    data=csv_bytes,
    file_name="historico_previsoes_depositos.csv",
    mime="text/csv"
)

st.caption("O histórico é guardado localmente em 'historico_previsoes_depositos.csv' na pasta da aplicação.")
